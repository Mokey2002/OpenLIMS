...
class InstrumentProfileViewSet(ModelViewSet):
    ...
    queryset = InstrumentProfile.objects.prefetch_related("column_mappings").all().order_by("name")


class InstrumentColumnMappingViewSet(ModelViewSet):
    permission_classes = [IsAdminOnly]
    serializer_class = InstrumentColumnMappingSerializer
    queryset = InstrumentColumnMapping.objects.select_related("instrument").all().order_by("instrument__name", "source_column")
...
class ImportJobViewSet(ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        try:
            ...
            reader = csv.DictReader(io.StringIO(decoded), delimiter=instrument.delimiter)
            ...
            for row in reader:
                ...
                for source_column, mapping in mappings.items():
                    ...
                    if mapping.value_type == "STRING":
                        result_kwargs["value_string"] = raw_value
                    elif mapping.value_type == "NUMBER":
                        try:
                            result_kwargs["value_number"] = float(raw_value)
                        except ValueError:
                            skipped_rows.append({
                                "row": rows_processed,
                                "sample_id": sample_code,
                        ...
                        result_kwargs["value_boolean"] = str(raw_value).strip().lower() in ["true", "1", "yes", "pass", "ok"]
