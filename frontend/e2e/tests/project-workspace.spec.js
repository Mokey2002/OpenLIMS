...
for (const role of ['admin', 'viewer']) {
  test(`project navigation and sample search for ${role}`, async ({ page }) => {
    await page.route('**/api/**', async route => {
      ...
      else if (path === '/api/projects/1/') body = { id: 1, code: 'TEST', name: 'Test project', members: [], member_usernames: [] };
      else if (path === '/api/samples/') body = [{ id: 1, sample_id: 'S-101', status: 'QC' }];
