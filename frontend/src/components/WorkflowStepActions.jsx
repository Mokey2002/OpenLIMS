...
export default function WorkflowStepActions({ value = {}, users, onChange }) {
  ...
  return <fieldset className="border rounded p-3 mt-2">
    ...
    <Form.Label htmlFor={assigneeId}>
      {t("Assign work to", "Asignar trabajo a")}
    </Form.Label>
      <Form.Select id={assigneeId} value={value.assigned_to || ""} onChange={e => update({ assigned_to: e.target.value ? Number(e.target.value) : null, notify_assignee: e.target.value ? !!value.notify_assignee : false })}>
    ...
    <Form.Label htmlFor={recipientsId} className="mt-2 d-block">
      {t("Additional in-app recipients", "Destinatarios adicionales en la aplicación")}
    </Form.Label>
      <Form.Select id={recipientsId} multiple value={(value.notify_users || []).map(String)} onChange={e => update({ notify_users: Array.from(e.target.selectedOptions, o => Number(o.value)) })}>
